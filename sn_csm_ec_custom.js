...
var SN_CSM_EC = (function () {
  ...
  var contextualRecommendationsShown = false,
    persistManualRecommendations = false,
    blockManualRecommendations = false,
    proactiveClosedAfterRefresh = false;
  ...
  function resetHomeFeatureFlag() {
    if (isHomeFeature) isHomeFeature = !isHomeFeature;
  }
  function addFeatureContext(featureContext) {
    if (!featureContext || !featureContext.feature || !featureContext.static)
      throw new Error("Unable to add new feature context");
    var contextObj = {};
  ...
  function prepareContext() {
    try {
      ...
      if (!featureData)
        throw new Error("Unable to identify feature " + featureObject.feature);
      ...
      Object.keys(featureObject.params).forEach(function (key) {
        ...
        if (propKey) contextObject[propKey] = featureObject.params[key];
        else {
          if (
            featureObject.feature == CHAT_FEATURE &&
            key.startsWith(CHAT_TOPIC_VAR_SUBSTRING)
          )
            contextObject[key] = featureObject.params[key];
          else
            throw new Error(
              key +
                " property is not registered in feature " +
      ...
      if (additionalUrlParams) urlParamArr.push(additionalUrlParams);
      Object.keys(contextObject).forEach(function (key) {
        if (contextObject[key] !== "") {
          urlParamArr.push(
            encodeURIComponent(key) +
              "=" +
              encodeURIComponent(contextObject[key])
          );
        }
      });
      if (featureObject.feature != CHAT_FEATURE)
        urlParamArr.push(
          encodeURIComponent("context_load") + "=" + encodeURIComponent("true")
        );
      ...
      if (
        featureObject &&
        featureObject.feature &&
        featureObject.openOnLoad == true
      )
        openOnloadFlag = true;
      else openOnloadFlag = false;
      CONTEXT = urlParam ? "?" + urlParam : CONTEXT;
    }
  }
  function addAttribute(param, index, array) {
    var keyVal = param.split("=");
    if (keyVal.length == 2) this[keyVal[0]] = keyVal[1];
  }
  function loadFeature() {
    ...
    if (returnVal.params.table == "sn_customerservice_case") {
      ...
      trimFeatureObject(
        returnVal.params,
        Object.keys(CONTEXT_FEATURE_MAP[returnVal.params.feature].params),
        returnVal.feature
      );
    }
    if (
      queryString.includes("feature=") &&
      CONTEXT_FEATURE_MAP[returnVal.feature]
    ) {
      isCalledFromDeepLinking = true;
  ...
  function trimFeatureObject(jsonObj, keysToRetain, feature) {
    for (var property in jsonObj) {
      if (jsonObj.hasOwnProperty(property)) {
        ...
        if (
          (feature != CHAT_FEATURE && keysToRetain.indexOf(keyString) == -1) ||
          (feature == CHAT_FEATURE &&
            !keyString.startsWith(CHAT_TOPIC_VAR_SUBSTRING) &&
            keysToRetain.indexOf(keyString) == -1)
        )
  ...
  function isRolesNeededBeforeInit(config) {
    if (
      config.loadFeature &&
      ROLES_NEEDING_FEATURES.includes(config.loadFeature.feature)
    ) {
      return true;
    } else {
      return false;
  ...
  function setConfig(configData) {
    if (configData) {
      ...
      if (configData.getAISRecommendationsContext)
        getProactiveRecommendationsQueryterm =
          configData.getAISRecommendationsContext;
      if (
        configData.enableRecommendations != null &&
        configData.enableRecommendations == false
      ) {
      ...
      EC_ORIGIN =
        ecOrigin[ecOrigin.length - 1] == "/"
          ? ecOrigin.slice(0, ecOrigin.length - 1)
          : ecOrigin;
      CONFIG_ID = ecDetails[1];
      if (configData.setLang || configData.lang) {
        IFRAME_LANG = "";
        if (configData.setLang) IFRAME_LANG = configData.setLang();
        if (
          IFRAME_LANG == undefined ||
          IFRAME_LANG == null ||
          IFRAME_LANG == ""
        )
  ...
  function loadConfig() {
    return fetchRequest({
      ...
      url:
        EC_ORIGIN +
        "/api/sn_csm_ec/engagement_center_api/modules/" +
        CONFIG_ID +
        (IFRAME_LANG != "" ? "?lang=" + IFRAME_LANG : ""),
    }).then(
      function (response) {
        if (response) {
          ...
          if (config.result && config.result.portal) {
            ...
            else config.result.sso_medium = "SAML";
  ...
  function stopAnimation() {
    var ecLauncherFloatingButton = document.getElementById(
  ...
  function injectAnimationAndTooltipElems(moduleConfigData) {
    ...
    var tooltipTemplate =
      '<div id="proactive-animation-tooltip-close-btn" tab-index="0" aria-label="Close tooltip messsage">' +
      '<svg  width="18" height="18" viewBox="0 0 16 16" fill="none" >' +
      '<path fill-rule="evenodd" clip-rule="evenodd" d="M3.35355 2.64642L2.64645 3.35353L7.26942 7.9765L2.64467 12.6482L3.35533 13.3517L7.97654 8.68362L12.6464 13.3535L13.3536 12.6464L8.68007 7.97294L13.2548 3.35174L12.5442 2.64821L7.97296 7.26583L3.35355 2.64642Z" fill="' +
      moduleConfigData.result.primary_text_color +
      '"/>' +
      "</svg>" +
      "</div>" +
      '<span class="proactive-animation-tooltip-text">' +
      moduleConfigData.result.tooltip_text +
      "</span>" +
      "<style>" +
      ...
      ";" +
      "background: " +
      moduleConfigData.result.primary_color +
      ";" +
      "border-radius: 7px;" +
      "position: fixed;" +
      "bottom: " +
    ...
    var ecEmbedWrapperDiv =
      document.getElementsByClassName("ecEmbedWrapper")[0];
    ecEmbedWrapperDiv.appendChild(tooltipDivElement);
    document
      .getElementById("proactive-animation-tooltip-close-btn")
      .addEventListener("click", closeToolTipWrapperFunc);
  }
  function openWidgetProactively(paramsObj) {
    if (
      contextualRecommendationsShown ||
      IFRAME_SRC.includes("id=recommendations&called_onload=true") &
        dummyPageOpen
    ) {
    ...
    if (proactiveHelpOpen) {
      if (POPUP_SHOW) {
        ...
        widgetFrame.contentWindow.postMessage(
          "RESET_LOCATION_AFTER_NOTIFICATION",
          EC_ORIGIN
        );
        POPUP_SHOW = false;
      }
        ...
        if (proactive_help_type == "offer_chat") {
          fetchRequest({
            ...
            url:
              EC_ORIGIN +
              "/api/sn_csm_ec/engagement_center_api/modules/" +
              moduleConfigData.result.portal,
            props: {
          ...
          if (contextualRecommendationsShown) {
            ...
            changeLastUrl(
              "recommendations&called_onload=true",
              "called_onload",
              "called_proactively"
            );
            if (POPUP_SHOW) {
            ...
            setTimeout(() => {
              ...
              widgetFrame.contentWindow.postMessage(
                {
                  type: "PROACTIVE_RECOMMENDATIONS_OPENED",
                  openedFromAnimation: false,
                },
                EC_ORIGIN
              );
            }, 1000);
            ...
            paramsObj = {
              type: "OPEN_PROACTIVE_OR_RECOMMENDATIONS",
              value:
                "?id=recommendations&spa=1&called_proactively=true&q=" +
                encodeURIComponent(
                  getProactiveRecommendationsQueryterm() || ""
                ) +
                "&context_load=true",
  ...
  function proactiveLoadCheck(moduleConfigData) {
    ...
    if (!proactiveEventListenersAdded) {
      var proactive_inactivity_time = parseInt(
        moduleConfigData.result.proactive_inactivity_time
      );
  ...
  function getProactiveRecommendationsQueryterm() {
    ...
    if (res == "") {
      if (str.length >= 2) res = str[str.length - 2];
    }
    return res;
  }
  function storeProactivehelpDismissActivity() {
    widgetFrame.contentWindow.postMessage(
      { type: "STORE_PROACTIVE_DISMISSED", page: window.location.href },
      EC_ORIGIN
    );
  }
  ...
  function customizeMessenger(data) {
    ...
    NOTIFICATION_COUNT_BG_COLOR =
      data.result.tertiary_color || NOTIFICATION_COUNT_BG_COLOR;
    NOTIFICATION_COUNT_TEXT_COLOR =
      data.result.tertiary_text_color || NOTIFICATION_COUNT_TEXT_COLOR;
    LAUNCHER_ICON_TOOLTIP =
      data.result.launcher_icon_tooltip || LAUNCHER_ICON_TOOLTIP;
  ...
  function updateImageDim() {
    ...
    img.onload = function () {
      ...
      if (dim["width"] > 60) dim["width"] = 60;
      if (dim["width"] < 24) dim["width"] = 24;
      if (dim["height"] > 60) dim["height"] = 60;
      if (dim["height"] < 24) dim["height"] = 24;
      ...
      document.getElementsByClassName("ecEmbedWidgetlauncher")[0].style.width =
        LAUNCHER_ICON_WIDTH + "px";
      document.getElementsByClassName("ecEmbedWidgetlauncher")[0].style.height =
        LAUNCHER_ICON_HEIGHT + "px";
      document.getElementsByClassName(
        "ecEmbedWidgetlauncher"
      )[0].style.marginLeft = LAUNCHER_ICON_MARGIN_LEFT + "px";
      document.getElementsByClassName(
        "ecEmbedWidgetlauncher"
      )[0].style.marginTop = LAUNCHER_ICON_MARGIN_TOP + "px";
      document.getElementsByClassName("ecEmbedWidgetlauncher")[0].src =
  ...
  function getTemplate(TEMPLATE_SRC) {
    return (
      ...
      ' title="' +
      LAUNCHER_ICON_TOOLTIP +
      '" alt="Messanger- launcher" decoding="async">' +
      '<img class="ecEmbedWidgetClose hidden" src=' +
      IFRAME_CLOSE_ICON_SRC +
      ' title="' +
      CLOSE_ICON_TOOLTIP +
      '" alt="Messanger- closed" decoding="async">' +
      '<div class="notificationCount hidden">' +
      '<span id="notificationCountValue">' +
      NOTIFICATION_COUNT +
      "</span>" +
      "</div>" +
      "</button>" +
      ...
      "top: 50%;" +
      "margin-left: " +
      LAUNCHER_ICON_MARGIN_LEFT +
      "px;" +
      "margin-top: " +
      LAUNCHER_ICON_MARGIN_TOP +
      "px;" +
      "z-index: 103;" +
      "}" +
      ...
      ";" +
      "bottom:" +
      (parseInt(IFRAME_V_PADDING) + parseInt(FLOATING_BUBBLE_SIZE) + 5) +
      "px;" +
      "max-height: calc(100% - " +
      (parseInt(IFRAME_V_PADDING) + parseInt(FLOATING_BUBBLE_SIZE) + 5) +
      "px);" +
    ...
    if (!openOnloadFlag && !wasLauncherOpened()) {
      ...
      compileTemplate(
        EC_ORIGIN +
          "/" +
          PORTAL_ID +
          "?id=em_dummy_page&context_load=true" +
          (IFRAME_LANG ? "&lang=" + IFRAME_LANG : "")
      );
    } else {
  ...
  function isMobilePlatform() {
    ...
    return new RegExp(
      /Android|iPhone|IEMobile|Windows Phone|BlackBerry|BB/gm
    ).test(userAgent);
  }
  function checkMobile() {
    setTimeout(function () {
      isMobilePlatform()
        ? widgetWrapper.classList.add("mobile")
        : widgetWrapper.classList.remove("mobile");
    }, 100);
  }
  ...
  function openWidget() {
    if (isProactiveAnimationRunning) {
      ...
      if (contextualRecommendationsShown || recommendationsFromAnimation) {
        ...
        changeLastUrl(
          "recommendations&called_onload=true",
          "called_onload",
          "called_proactively=true&recommendationsFromAnimation"
        );
        widgetFrame.contentWindow.postMessage(
          {
            type: "PROACTIVE_RECOMMENDATIONS_OPENED",
            openedFromAnimation: true,
          },
          EC_ORIGIN
        );
      } else {
        ...
        widgetFrame.contentWindow.postMessage(
          {
            type: "OPEN_PROACTIVE_OR_RECOMMENDATIONS",
            value:
              "?id=recommendations&spa=1&called_proactively=true&recommendationsFromAnimation=true&q=" +
              encodeURIComponent(getProactiveRecommendationsQueryterm() || "") +
              "&context_load=true",
          },
          EC_ORIGIN
        );
    ...
    if (dummyPageOpen) {
      widgetFrame.contentWindow.postMessage(
        {
          type: "LEAVE_DUMMY_PAGE",
          new_page: IFRAME_SRC.substring(
            IFRAME_SRC.lastIndexOf("/"),
            IFRAME_SRC.length
          ),
        },
        EC_ORIGIN
    ...
    isMobilePlatform() || isLastDockStateOUT
      ? dockOutMessenger()
      : dockInMessenger();
    showMessengerFrame();
    ...
    widgetFrame.contentWindow.postMessage(
      { type: "EC_STATUS", status: "opened" },
      EC_ORIGIN
    );
    widgetFrame.focus();
    notificationBubble.classList.add("hidden");
    if (!POPUP_SHOW)
      widgetFrame.contentWindow.postMessage("EC_OPENED!", EC_ORIGIN);
    else {
      POPUP_SHOW = false;
    }
    if (baseUrl)
      widgetFrame.contentWindow.postMessage(
        { type: "baseUrl", value: baseUrl },
        EC_ORIGIN
      );
  }
  ...
  function onExternalUserLogin() {
    ...
    onLoadConfig.then(function (SSOType) {
      ...
      if (SSOType === "OIDC") {
        ...
        !isLoggedInUser &&
          document.body.addEventListener("click", authenticateViaSAML);
        initilizeWidget();
        closeWidget();
      }
    });
  ...
  function sanitizeIframeSrcForLang() {
    ...
    if (langIndex != -1) {
      IFRAME_SRC =
        IFRAME_SRC.substring(0, langIndex) +
        IFRAME_SRC.substring(langIndex + 8, IFRAME_SRC.length);
    }
  }
      ...
      fetchTokenCallback().then(function (token) {
        if (token) {
          ...
          if (isMFAEnabledOnInstance) {
            ...
            oidcFrame.innerHTML = compileOIDCFormIframe(
              EC_ORIGIN +
                "/navpage.do?redirect_uri=" +
  ...
  function authenticateViaSAML() {
    if (!isLoggedInUser) {
      ...
      popupObjectReference = !isLoggedInUser
        ? window.open(
            EC_ORIGIN + "/sn_csm_ec_saml_auth_landing_page.do",
            "login",
            popupFeature
          )
        : null;
  ...
  function getLastVisitedUrl() {
    var lastUrl = "";
    var lastVisitedUrlInfo = localStorage.getItem(
      STORAGE_LAST_NAVIGATION_DATA_KEY
    );
  ...
  function updateLastUrl(url) {
    ...
    localStorage.setItem(
      STORAGE_LAST_NAVIGATION_DATA_KEY,
      JSON.stringify(lastNavigationData)
    );
  }
  function changeLastUrl(url1, url2, url3) {
    var lastNavigationData = localStorage.getItem(
      STORAGE_LAST_NAVIGATION_DATA_KEY
    )
      ? JSON.parse(localStorage.getItem(STORAGE_LAST_NAVIGATION_DATA_KEY))
      : {};
  ...
  function isLastUrlIntermediate() {
    if (
      IFRAME_SRC.includes("id=recommendations") ||
      IFRAME_SRC.includes("id=proactive_search_page") ||
      IFRAME_SRC.includes("id=proactive_chat_page")
    ) {
      isIntermediatePageOpen = false;
  ...
  function manualRecommendationsCheck() {
    if (
      moduleRespData.result.enable_recommendations == "true" &&
      !proactiveHelpOpen &&
      !isCalledFromDeepLinking &&
      enableRecommendations &&
      !isIntermediatePageOpen &&
      !blockManualRecommendations
    ) {
      if (IFRAME_LANG == "") {
        IFRAME_SRC =
          EC_ORIGIN +
          "/" +
          PORTAL_ID +
          "?id=recommendations&called_onload=true&spa=1&q=" +
          encodeURIComponent(getProactiveRecommendationsQueryterm() || "");
      } else {
        IFRAME_SRC =
          EC_ORIGIN +
  ...
  window.addEventListener(
    "message",
    function (e) {
      if (e.origin !== EC_ORIGIN) return;
      switch (e.data.type) {
        case "SYC": {
          isLoggedInUser = e.data.status;
          if (e.data.proactiveHelpDismissedPages) {
            isProactiveHelpDismissed =
              !!e.data.proactiveHelpDismissedPages[window.location.href];
            if (
              Object.keys(e.data.proactiveHelpDismissedPages).length >=
              proactiveHelpDismissThreshold
            ) {
          ...
          widgetFrame.contentWindow.postMessage(
            {
              type: "SYC_ACK!",
              status: emStatus,
              dockedOut: !isLastDockStateOUT,
              dummyPageOpen: dummyPageOpen,
              originalIframeSrc: IFRAME_SRC.substring(
                IFRAME_SRC.lastIndexOf("/") + 1,
                IFRAME_SRC.length
              ),
            },
            EC_ORIGIN
          ...
          widgetFrame.contentWindow.postMessage(
            { type: "NAV_CHECK", currURL: e.data.currUrl },
            EC_ORIGIN
          );
          break;
        }
        ...
        case "CLOSE_MESSENGER": {
          ...
          if (
            prevPageBeforePopup == "" ||
            prevPageBeforePopup == CONFIG_ID ||
            prevPageBeforePopup.includes("id=recommendations") ||
            prevPageBeforePopup.includes("id=proactive_search_page") ||
            prevPageBeforePopup.includes("id=proactive_chat_page") ||
            prevPageBeforePopup.includes("id=em_dummy_page")
          ) {
            isIntermediatePageOpen = false;
          } else {
            isIntermediatePageOpen = true;
        ...
        case "UPDATE_NOTIFICATION_COUNT": {
          ...
          if (NOTIFICATION_COUNT === "0")
            ...
            document.getElementById("notificationCountValue").textContent =
              NOTIFICATION_COUNT;
          if (
            IS_WIDGET_COLLAPSED &&
            NOTIFICATION_COUNT != "" &&
        ...
        case "OPEN_PROACTIVE_WIDGET": {
          if (e.data.height && !POPUP_SHOW) {
            ...
            isLastDockStateOUT = false;
            dockInMessenger();
            if (!wasLauncherOpened()) openWidget();
          }
        ...
        case "CLOSE_PROACTIVE_HELP": {
          if (recommendationsFromAnimation) {
            if (persistManualRecommendations) {
              changeLastUrl(
                "recommendations&called_proactively=true",
                "recommendationsFromAnimation=true&",
                ""
              );
              changeLastUrl(
              ...
              widgetFrame.contentWindow.postMessage(
                { type: "MANUAL_RECOMMENDATIONS_OPENED" },
                EC_ORIGIN
              );
              ...
              widgetFrame.contentWindow.postMessage(
                { type: "REMOVE_PROACTIVE_RECOMMENDATIONS_HISTORY" },
                EC_ORIGIN
              );
            }
            ...
            storeProactivehelpDismissActivity();
            if (
              persistManualRecommendations &&
              moduleRespData.result.proactive_help_type ==
              ...
              persistManualRecommendations &&
              (moduleRespData.result.proactive_help_type == "offer_chat" ||
                moduleRespData.result.proactive_help_type == "offer_search")
            ) {
              widgetFrame.contentWindow.postMessage(
                {
                  type: "OPEN_PROACTIVE_OR_RECOMMENDATIONS",
                  value:
                    "?id=recommendations&spa=1&called_onload=true&q=" +
                    encodeURIComponent(
                      getProactiveRecommendationsQueryterm() || ""
                    ) +
                    "&context_load=true",
  ...
  function getUserRolesAndInitWidget(isLoginAttempted, config) {
    fetchRequest({
      ...
      if (response) {
        ...
        CONTEXT_FEATURE_MAP["NEW_CASE"].static["sys_id"] =
          NEW_CASE_CAT_ITEM_SYS_ID;
      }
      if (isLoginAttempted && USER_TYPE != "guest") {
        ...
        if (isHomeFeature) {
          ...
          loadLastURL &&
          !(
            lastVisitedUrl.includes("recommendations") ||
            lastVisitedUrl.includes("proactive")
          )
        ) {
          IFRAME_SRC = lastVisitedUrl;
          isLastUrlIntermediate();
  ...
  function initialiseModule(config) {
    setConfig(config).then(function () {
      ...
      if (
        contextFunction &&
        contextFunction.feature &&
        contextFunction.openOnLoad
      )
        openWidget();
